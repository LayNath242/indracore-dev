...
use indracore_primitives::v1::{Block as PBlock, CollatorId, CollatorPair};
use indracore_service::{AbstractClient, Client as PClient, ClientHandle, RuntimeApiCollection};
...
/// Indracore full node handles.
type PFullNode<C> = indracore_service::NewFull<C>;
...
pub struct StartCollatorParams<'a, Block: BlockT, PF, BI, BS, Client, Backend, Spawner, PClient, PBackend> {
	...
	pub indracore_full_node: PFullNode<PClient>,
	pub task_manager: &'a mut TaskManager,
	pub indracore_backend: Arc<PBackend>,
}
	...
	PBackend::State: StateBackend<BlakeTwo256>,
{
	indracore_full_node
		...
		.execute_with(StartCollator {
			...
			client,
			announce_block,
			overseer_handler: indracore_full_node
				.overseer_handler
				.ok_or_else(|| "Indracore full node did not provided an `OverseerHandler`!")?,
			spawner,
			para_id,
			collator_key,
			block_import,
	...
	task_manager.add_child(indracore_full_node.task_manager);
...
struct StartCollator<Block: BlockT, Client, Backend, PF, BI, BS, Spawner, PBackend> {
	...
	indracore_backend: Arc<PBackend>,
}

impl<Block, Client, Backend, PF, BI, BS, Spawner, PBackend2> indracore_service::ExecuteWithClient
		...
		PClient: AbstractClient<PBlock, PBackend, Api = Api> + 'static,
	{
		async move {
			cumulus_collator::start_collator(cumulus_collator::StartCollatorParams {
				...
				indracore_client: client,
				indracore_backend: self.indracore_backend,
			})
			.await
			.map_err(Into::into)
		}
...
where
	...
	PClient: ClientHandle,
{
	indracore_full_node.client.execute_with(StartFullNode {
		announce_block,
		para_id,
		client,
		task_manager,
...
impl<'a, Block, Client, Backend> indracore_service::ExecuteWithClient
	...
	Backend: BackendT<Block> + 'static,
{
	...
	where
		...
		PClient: AbstractClient<PBlock, PBackend, Api = Api> + 'static,
	{
		let future = cumulus_consensus::follow_indracore(
			self.para_id,
			self.client,
			client,
			self.announce_block,
...
/// Build the Indracore full node using the given `config`.
#[sc_cli::prefix_logs_with("Relaychain")]
pub fn build_indracore_full_node(
	config: Configuration,
	collator_id: CollatorId,
) -> Result<PFullNode<PClient>, indracore_service::Error> {
	let is_light = matches!(config.role, Role::Light);
	if is_light {
		Err(indracore_service::Error::Sub(
			"Light client not supported.".into(),
		))
	} else {
		indracore_service::build_full(
			config,
			indracore_service::IsCollator::Yes(collator_id),
			None,
			None,
		)
	}
