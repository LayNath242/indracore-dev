...
/// Helper for the Indracore client. This is expected to be a lightweight handle
/// like an `Arc`.
pub trait IndracoreClient: Clone + 'static {
...
/// Spawns a future that follows the Indracore relay chain for the given parachain.
pub fn follow_indracore<L, P, Block, B>(
	para_id: ParaId,
	local: Arc<L>,
	indracore: P,
	...
	B: Backend<Block>,
{
	...
	Ok(future::select(
		follow_finalized,
		follow_new_best(para_id, local, indracore, announce_block)?,
	)
	.map(|_| ()))
}
...
impl<T> IndracoreClient for Arc<T>
where
...
where
	Block: BlockT,
	PC: IndracoreClient + Clone + Send + Sync,
	PC::Error: ToString,
