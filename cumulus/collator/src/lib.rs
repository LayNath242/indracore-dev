...
use indracore_service::RuntimeApiCollection;
...
pub struct Collator<Block: BlockT, PF, BI, BS, Backend, PBackend, PClient, PBackend2> {
	...
	backend: Arc<Backend>,
	indracore_client: Arc<PClient>,
	indracore_backend: Arc<PBackend2>,
}
	...
	for Collator<Block, PF, BI, BS, Backend, PBackend, PClient, PBackend2>
{
	fn clone(&self) -> Self {
		Self {
			...
			indracore_backend: self.indracore_backend.clone(),
		}
	}
}
...
where
	...
	PClient: indracore_service::AbstractClient<PBlock, PBackend, Api = PApi> + 'static,
...
pub struct StartCollatorParams<Block: BlockT, PF, BI, Backend, Client, BS, Spawner, PClient, PBackend> {
	...
	pub indracore_client: Arc<PClient>,
...
where
	...
	PBackend2::State: StateBackend<BlakeTwo256>,
{
	let follow = match cumulus_consensus::follow_indracore(
		para_id,
		client,
		indracore_client.clone(),
		announce_block.clone(),
	) {
		Ok(follow) => follow,
		Err(e) => return Err(format!("Could not start following indracore: {:?}", e)),
	};

	spawner.spawn("cumulus-follow-indracore", follow.map(|_| ()).boxed());
...
mod tests {
	...
	use indracore_node_subsystem::messages::CollationGenerationMessage;
	...
	fn collates_produces_a_block() {
		...
		let (indracore_client, indracore_backend, relay_parent) = {
			// Create a indracore client with a block imported.
			use indracore_test_client::{
				ClientBlockImportExt as _, DefaultTestClientBuilderExt as _,
				InitIndracoreBlockBuilder as _, TestClientBuilderExt as _,
			};

			let client_builder = indracore_test_client::TestClientBuilder::new();
			let indracore_backend = client_builder.backend();
			...
			(client, indracore_backend, hash)
		};

		let collator_start =
			start_collator::<_, _, _, _, _, _, _, _, indracore_service::FullBackend, _, _>(
				StartCollatorParams {
					...
					indracore_client: Arc::new(indracore_client),
					indracore_backend,
				},
			);
