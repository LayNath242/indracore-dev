		...
		+ 'static,
{
	...
	let indracore_full_node = indracore_test_service::new_full(
		indracore_config,
		indracore_service::IsCollator::Yes(collator_key.public()),
	)
	.map_err(|e| match e {
	...
	let indracore_full_node = indracore_full_node.with_client(indracore_test_service::TestClient);
	if is_collator {
		...
		let indracore_backend = indracore_full_node.backend.clone();
...
pub async fn run_test_node(
	...
	indracore_storage_update_func: impl Fn(),
	parachain_boot_nodes: Vec<MultiaddrWithPeerId>,
	indracore_boot_nodes: Vec<MultiaddrWithPeerId>,
	para_id: ParaId,
	is_collator: bool,
) -> CumulusTestNode {
	...
	let mut indracore_config = indracore_test_service::node_config(
		indracore_storage_update_func,
		task_executor.clone(),
		key,
		indracore_boot_nodes,
		false,
	);

	indracore_config.network.node_name =
		format!("{} (relay chain)", indracore_config.network.node_name);
