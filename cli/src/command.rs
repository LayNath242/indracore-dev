...
impl SubstrateCli for Cli {
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		...
		Ok(match id {
			...
			"xelendra-staging" => Box::new(service::chain_spec::xelendra_staging_testnet_config()?),
			"xelendra-local" => Box::new(service::chain_spec::xelendra_local_testnet_config()?),
			"xelendra" => Box::new(service::chain_spec::xelendra_config()?),
			path => {
				...
				if self.run.force_xelendra || starts_with("xelendra") {
					Box::new(service::XelendraChainSpec::from_json_file(path)?)
				} else {
	...
	fn native_runtime_version(spec: &Box<dyn service::ChainSpec>) -> &'static RuntimeVersion {
		if spec.is_xelendra() {
			&service::xelendra_runtime::VERSION
		} else {
	...
	match &cli.subcommand {
		...
		Some(Subcommand::Revert(cmd)) => {
			...
			Ok(runner.async_run(|mut config| {
				...
				Ok(
					(cmd.run(client, backend).map_err(Error::SubstrateCli),task_manager)
				)})?)
		},
