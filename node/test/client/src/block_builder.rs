...
use indracore_test_runtime::{GetLastTimestamp, UncheckedExtrinsic};
...
pub trait InitIndracoreBlockBuilder {
	...
	fn init_indracore_block_builder(&self) -> sc_block_builder::BlockBuilder<Block, Client, FullBackend>;

	/// Init a Indracore specific block builder at a specific block that works for the test runtime.
	///
	/// Same as [`InitIndracoreBlockBuilder::init_indracore_block_builder`] besides that it takes a [`BlockId`] to say
	/// which should be the parent block of the block that is being build.
	fn init_indracore_block_builder_at(
		&self,
		at: &BlockId<Block>,
...
impl InitIndracoreBlockBuilder for Client {
	fn init_indracore_block_builder(
		&self,
	) -> BlockBuilder<Block, Client, FullBackend> {
		let chain_info = self.chain_info();
		self.init_indracore_block_builder_at(&BlockId::Hash(chain_info.best_hash))
	}
...
/// Indracore specific extensions for the [`BlockBuilder`].
pub trait BlockBuilderExt {
	/// Push a Indracore test runtime specific extrinsic to the block.
	///
	...
	fn push_indracore_extrinsic(&mut self, ext: UncheckedExtrinsic) -> Result<(), sp_blockchain::Error>;
}
