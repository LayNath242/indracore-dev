...
use indracore_runtime_common::BlockHashCount;
use indracore_service::{
	Error, NewFull, FullClient, ClientHandle, ExecuteWithClient, IsCollator,
};
use indracore_node_subsystem::messages::{CollatorProtocolMessage, CollationGenerationMessage};
use indracore_test_runtime::{
	Runtime, SignedExtra, SignedPayload, VERSION, ParasSudoWrapperCall, SudoCall, UncheckedExtrinsic,
};
use indracore_node_primitives::{CollatorFn, CollationGenerationConfig};
use indracore_runtime_parachains::paras::ParaGenesisArgs;
...
native_executor_instance!(
	pub IndracoreTestExecutor,
	indracore_test_runtime::api::dispatch,
	indracore_test_runtime::native_version,
	frame_benchmarking::benchmarking::HostFunctions,
);

/// The client type being used by the test service.
pub type Client = FullClient<indracore_test_runtime::RuntimeApi, IndracoreTestExecutor>;

pub use indracore_service::FullBackend;
...
pub fn new_full(
	...
	Error,
> {
	indracore_service::new_full::<indracore_test_runtime::RuntimeApi, IndracoreTestExecutor>(
		config,
		is_collator,
		None,
		None,
		indracore_parachain::wasm_executor::IsolationStrategy::InProcess,
	)
}
...
impl ClientHandle for TestClient {
	fn execute_with<T: ExecuteWithClient>(&self, t: T) -> T::Output {
		T::execute_with_client::<_, _, indracore_service::FullBackend>(t, self.0.clone())
	}
}

/// Create a Indracore `Configuration`.
///
...
pub fn node_config(
	...
	let mut spec = indracore_local_testnet_config();
	...
	Configuration {
		impl_name: "indracore-test-node".to_string(),
...
pub fn run_validator_node(
	...
	let NewFull { task_manager, client, network, rpc_handlers, overseer_handler, .. } =
		new_full(config, IsCollator::No).expect("could not create Indracore test service");
...
/// [`IndracoreTestNode::register_collator`].
pub fn run_collator_node(
	...
	let multiaddr = config.network.listen_addresses[0].clone();
	let NewFull { task_manager, client, network, rpc_handlers, overseer_handler, .. } =
		new_full(config, IsCollator::Yes(collator_id)).expect("could not create Indracore test service");
...
/// A Indracore test node instance used for testing.
pub struct IndracoreTestNode {
...
impl IndracoreTestNode {
	/// Send an extrinsic to this node.
	pub async fn send_extrinsic(
		&self,
		function: impl Into<indracore_test_runtime::Call>,
...
pub fn construct_extrinsic(
	...
	UncheckedExtrinsic::new_signed(
		function.clone(),
		indracore_test_runtime::Address::Id(caller.public().into()),
		indracore_primitives::v0::Signature::Sr25519(signature.clone()),
		extra.clone(),
	)
}
...
pub fn construct_transfer_extrinsic(
	...
	let function = indracore_test_runtime::Call::Balances(
