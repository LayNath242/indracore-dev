...
pub use chain_spec::{IndracoreChainSpec, XelendraChainSpec};
...
native_executor_instance!(
	pub XelendraExecutor,
	xelendra_runtime::api::dispatch,
	xelendra_runtime::native_version,
	frame_benchmarking::benchmarking::HostFunctions,
);
		...
		Executor: NativeExecutionDispatch + 'static,
{
	...
	config.network.notifications_protocols.extend(indracore_network_bridge::notifications_protocol_info());
...
>
{
	...
	if config.chain_spec.is_xelendra() {
		let service::PartialComponents { client, backend, import_queue, task_manager, .. }
			= new_partial::<xelendra_runtime::RuntimeApi, XelendraExecutor>(config, jaeger_agent)?;
		Ok((Arc::new(Client::Xelendra(client)), backend, import_queue, task_manager))
	} else {
		let service::PartialComponents { client, backend, import_queue, task_manager, .. }
...
pub fn build_light(config: Configuration) -> Result<(TaskManager, RpcHandlers), Error> {
	if config.chain_spec.is_xelendra() {
		new_light::<xelendra_runtime::RuntimeApi, XelendraExecutor>(config)
	} else {
...
pub fn build_full(
	...
	if config.chain_spec.is_xelendra() {
		new_full::<xelendra_runtime::RuntimeApi, XelendraExecutor>(
			config,
			is_collator,
			grandpa_pause,
			jaeger_agent,
			Default::default(),
		).map(|full| full.with_client(Client::Xelendra))
	} else {
